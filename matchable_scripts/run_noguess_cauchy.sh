...
stats_directory_base=${stats_dir}/stats_noguess
...
echo -e running: ${UCYAN}Levemberg-Marquardt with Cauchy kernel no-guess${NC}
cd ${target_dir}
echo $'\n'
...
for d in "${noise_directories[@]}"; do
  ...
  for f in "${files[@]}"; do
    ...
    ${G2O_ROOT}/bin/g2o -v -i ${iterations} -statsAdvanced -solver lm_var_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -stats ${stats_file} -summary ${summary_file} -o ${output_file} ${f}
    echo $'\n'
  done
