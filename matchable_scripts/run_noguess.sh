...
for f in "${files[@]}"; do
  ...
  f_base=${f##*/}
  f_prefix=${f_base%.*}
  output_file=output/${f_prefix}_output.g2o
  stats_file=output/${f_prefix}.stats
  echo -e output graph: ${UYELLOW}${output_file}${NC}
  echo -e stats file: ${UYELLOW}${stats_file}${NC}


  ${G2O_ROOT}/bin/g2o -v -i 500 -solver lm_var_cholmod -stats ${stats_file} -o ${output_file} ${f}
  echo $'\n'
done
