...
for d in "${noise_directories[@]}"; do
  ...
  for f in "${files[@]}"; do
    ...
    ${G2O_ROOT}/bin/g2o -v -i ${iterations} -guessOdometry -solver lm_var_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -stats ${stats_file} -summary ${summary_file} -o ${output_file} ${f}
    echo $'\n'
  done
