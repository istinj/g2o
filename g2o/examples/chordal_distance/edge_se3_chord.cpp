...
namespace g2o {
  namespace chordal {
    ...
    bool EdgeSE3Chord::read(std::istream& is) {
      ...
      _measurement = chordal::v2tQUAT(meas);
    ...
    bool EdgeSE3Chord::write(std::ostream& os) const {
      Vector7 meas = chordal::t2vQUAT(_measurement);
      for (uint8_t i = 0; i < 7; ++i) {
        os << meas[i] << " ";
      }
    ...
    void EdgeSE3Chord::computeError() {
      ...
      _error = chordal::flattenIsometry(delta);
    }
    ...
    void EdgeSE3Chord::linearizeOplus() {
      ...
      _jacobianOplusXj.block<3,3>(9,3) = -Ri.transpose() * chordal::skew(tj);
