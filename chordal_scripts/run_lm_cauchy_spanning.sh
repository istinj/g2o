...
for f in "${files[@]}"; do
  ...
  ${G2O_ROOT}/bin/g2o_chordal_app -v -i 100 -guess -solver lm_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -stats ${stats_file_geodesic} -summary ${summary_file_geodesic} -o ${output_file_geodesic} ${f}

  echo $'\n'
  echo -e ${YELLOW}chordal${NC}
  ${G2O_ROOT}/bin/g2o_chordal_geodesic_comparator -i 101 -guess -solver lm_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -o ${output_file_chordal} -compareStats ${stats_file_chordal} -summary ${summary_file_chordal} -geodesicGraph ${f} chordal/${f}
