...
echo -e running: ${UCYAN}Gauss-Newton odometry${NC}
cd ${target_dir}
echo $'\n'
...
for f in "${files[@]}"; do
  ...
  ${G2O_ROOT}/bin/g2o_chordal_app -v -i 100 -guessOdometry -solver gn_fix6_3_cholmod -stats ${stats_file_geodesic} -summary ${summary_file_geodesic} -o ${output_file_geodesic} ${f}

  echo $'\n'
  echo -e ${YELLOW}chordal${NC}
  ${G2O_ROOT}/bin/g2o_chordal_geodesic_comparator -dynamicEpsilon -i 101 -guessOdometry -solver gn_fix6_3_cholmod -o ${output_file_chordal} -compareStats ${stats_file_chordal} -summary ${summary_file_chordal} -geodesicGraph ${f} chordal/${f}
