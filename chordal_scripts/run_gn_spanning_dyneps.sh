...
echo -e running: ${UCYAN}Gauss-Newton spanning${NC}
cd ${target_dir}
echo $'\n'
...
for f in "${files[@]}"; do
  ...
  ${G2O_ROOT}/bin/g2o_chordal_geodesic_comparator -dynamicEpsilon -i 101 -guess -solver gn_fix6_3_cholmod -o ${output_file_chordal} -compareStats ${stats_file_chordal} -summary ${summary_file_chordal} -geodesicGraph ${f} chordal/${f}
