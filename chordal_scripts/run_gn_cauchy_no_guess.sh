...
statics_directory_base=${stats_dir}/stats_noguess
...
echo -e running: ${UCYAN}Gauss-Newton no-guess${NC}
cd ${target_dir}
echo $'\n'


#ia directories for the outputs
output_directory_geodesic=output_noguess
...
output_directory_chordal=chordal/output_noguess
...
for f in "${files[@]}"; do
  ...
  ${G2O_ROOT}/bin/g2o -v -i 100 -solver gn_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -stats ${stats_file_geodesic} -o ${output_file_geodesic} ${f}

  echo $'\n'
  echo -e ${YELLOW}chordal${NC}
  ${G2O_ROOT}/bin/chordal_comparator -i 100 -solver gn_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -o ${output_file_chordal} -compareStats ${stats_file_chordal} -otherGraph ${f} chordal/${f}
...
for f in "${files[@]}"; do
  ...
  #ia process the standard and the chordal one
  echo -e ${YELLOW}standard${NC}
  ${G2O_ROOT}/bin/g2o -v -i 100 -solver gn_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -stats ${stats_file_standard} -o ${output_file_standard} ${f}
  
  # echo $'\n'
  # echo -e ${YELLOW}chordal${NC}
  # ${G2O_ROOT}/bin/g2o -v -i 100 -solver gn_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -stats ${stats_file_chordal} -o ${output_file_chordal} chordal/${f}
  
  # echo $'\n'
  # echo -e ${YELLOW}compare${NC}
  # ${G2O_ROOT}/bin/chordal_comparator -i 100 -solver gn_fix6_3_cholmod -robustKernel Cauchy -robustKernelWidth 1.0 -compareStats ${stats_file_compare} -otherGraph ${f} chordal/${f}
